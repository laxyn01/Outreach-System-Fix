...
def upload():
    ...
    if request.method == 'POST':
        ...
        if action == 'preview' and 'file' in request.files:
            ...
            if rows_json:
                import json
                rows_json = rows_json.strip() if rows_json else '[]'
                for row in rows:
                    email = clean_email(row.get('email', ''))
                    if not email:
                        skipped_confirm += 1
                    ...
                    imported += 1
                db.session.commit()
            flash(f'Imported {imported} leads. Skipped {skipped_confirm} invalid emails.', 'success')
            return redirect(url_for('upload'))

    return render_template('upload.html', preview_rows=[], skipped=0, show_preview=False)
...
def settings_page():
    ...
    active_days = (settings.active_days or '').split(',')
    return render_template(
        'settings.html',
        settings=settings,
        timezones=timezones,
