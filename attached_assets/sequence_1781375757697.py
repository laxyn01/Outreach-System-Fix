...
def get_available_accounts(settings: Settings) -> list[EmailAccount]:
    limit = settings.daily_limit_per_account or 15
    accounts = EmailAccount.query.filter_by(warmup_enabled=False).all()
    available = []
    for acc in accounts:
    ...
    if not available and accounts:
        ...
        db.session.commit()
        available = [a for a in accounts if a.daily_sent_count < limit]
    return available


def pick_account(accounts: list[EmailAccount], lead: Lead) -> EmailAccount | None:
    if not accounts:
        return None
